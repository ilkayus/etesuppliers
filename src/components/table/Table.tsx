...
const Table = ({ selected, setSelected, fetchFn, type }: Props) => {
  ...
  const tableColumnClickEventHandler = (e: any) => {
    ...
    const idx = dataKeys.indexOf(sortBy);
    //console.log(sortBy, data[0][sortBy]);
    dispatch({ type: "setFilter", payload: idx });
    dispatch({ type: "setData", payload: idx });
    setFilter((prev) =>
      prev.map((el, index) => (index === idx ? (el === 2 ? 1 : 2) : 0))
    );
    setData((prev) =>
  ...
  return (
    <div className="table-table">
      <div className="per-page">
        <img
          className={"left-arrow " + state.leftArrow}
          src={icons.upArrow}
          alt=""
          onClick={() => {
            dispatch({ type: "decrementPageNumber", payload: undefined });
          }}
        />
        <select
          name="post-per-page"
        ...
        >
          <option value="5">5 Results - {`Page ${state.currentPage}`}</option>
          <option value="10">10 Results {`Page ${state.currentPage}`}</option>
          <option value="20">20 Results {`Page ${state.currentPage}`}</option>
        </select>
        <img
          className={"right-arrow " + state.rightArrow}
          src={icons.upArrow}
          alt=""
          onClick={() => {
            dispatch({ type: "incrementPageNumber", payload: undefined });
          }}
        />
      </div>
      {jsxTable}
